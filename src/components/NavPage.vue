<template>
    <div class="w-full py-5 z-20 fixed top-0" :class="this.$route.path !== '/home' && 'bg-white'">
        <div class="max-w-7xl mx-auto flex justify-between items-center px-10">
            ...
            <div :class="isOpen === true && '!left-0'" class="w-52 pl-10 border-r-2 border-nutty bg-white h-full z-30 top-0 transiton-all duration-500 -left-60 fixed">
                <ul class="mt-28">
                    <li v-for="(e, index) in NavList[0]" :key="e" class="mb-12">
                        ...
                        <ul v-if="index === 0" class="submenu ml-5">
            ...
            <h1 class="basis-10/12"><router-link :to="this.$store.state.loginChk === true ? '/': '/home'"><img src="@/assets/logo.png" class="w-16"></router-link></h1>
            <ul class="basis-1/12 flex justify-end gap-x-4">
                <li v-if="this.$store.state.loginChk === false"><router-link to="/login" @click="isOpen=false">로그인</router-link></li>
                <li v-else><router-link to="/mypage" @click="isOpen=false">{{this.$store.state.displayName}}</router-link></li>
            </ul>
        </div>
    </div>
</template>
