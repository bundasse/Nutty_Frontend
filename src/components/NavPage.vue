<template>
    <div class="max-w-7xl mx-auto py-10">
        <div class="top-10 z-50 pt-5 left-10 fixed" @click="isOpen == true ? isOpen = false : isOpen = true" :class="isOpen && 'on'">
            <div v-for="e in 3" :key="e" class="h-0.5 w-7 bg-black rounded transition-all duration-500 m-1.5 top-5" :class="isOpen && ' on'"></div>
        </div>
        <div :class="isOpen === true && '!left-0'" class="w-80 bg-white h-full z-30 top-0 transiton-all duration-500 -left-80 fixed">
            <ul class="mt-20">
                <li v-for="(e, index) in NavList[0]" :key="e" v-on:click="NavSelectIndex = index; SubDown(index)" class="text-center mb-12">
                    <router-link :to="NavList[1][index]" class="hover:text-amber-600 hover:font-bold relative after:absolute after:h-[1px] after:bg-gray-800 after:w-[80px] after:left-1/2 after:-translate-x-1/2 after:bottom-0 text-center">{{ e }}</router-link>
                    <ul v-if="index === 2" class="submenu">
                    <li v-for="(el, i) in SubList" :key="el"><router-link :to="SubMenuLink[i]"
                        class="hover:text-amber-600 hover:font-bold"> {{ el }}</router-link></li>
                    </ul>
                </li>
            </ul>
