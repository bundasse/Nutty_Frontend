<template>
    <div class="mt-32 max-w-5xl mx-auto">
        ...
        <p class="flex justify-between border-b border-gray-300">
            <span class="text-gray-500">{{ writer }}</span>
            <ul class="flex gap-x-3">
                <li>{{ createDate }}</li>
                <li>조회수</li>
                <li>댓글수 {{ commentList.length }}</li>
                <li>좋아요 수</li>
            </ul>
        </p>
        ...
        <div class="flex items-start justify-between border p-5 rounded-lg">
            ...
            <button @click="reply" class="basis-2/12 border px-2 py-1 cursor-pointer hover:bg-nutty rounded-sm">등록</button>
        </div>
    </div>
</template>
<script>
